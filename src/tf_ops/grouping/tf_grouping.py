...
def unique_1nn_point(xyz1, xyz2):
    ...
    xyz1 = tf.tile(tf.reshape(xyz1, (b,1,n,c)), [1,m,1,1])
    xyz2 = tf.tile(tf.reshape(xyz2, (b,m,1,c)), [1,1,n,1])
    var_output = tf.reduce_sum((xyz1-xyz2)**2, -1)

    dist = tf.Variable(0, dtype=var_output.dtype,
                       trainable=False, collections=[])

    dist = tf.assign(dist, var_output, validate_shape=False)

    idx_list = []
    for new_xyz in range(m):
        idx = tf.to_int32(tf.argmin(dist[:, new_xyz, :], axis=-1))
        if new_xyz != m-1:
            indices = tf.constant([[k, i] for i in range(new_xyz+1,
                                                         m) for k in range(b)], dtype=tf.int32) #
            repeated_idx = tf.expand_dims(tf.tile(idx, [m - new_xyz - 1]), 1)
            new_indices = tf.concat([indices, repeated_idx], axis=1)
            updates = tf.constant([float('Inf') for i in range(new_xyz+1,
                                                               m) for k in range(b)])
            dist = tf.scatter_nd_update(dist, new_indices, updates)
        if new_xyz % 50 == 0:
            print(new_xyz)
        idx_list.append(idx)

    # m x b => b x m
    return tf.expand_dims(tf.transpose(tf.stack(idx_list)), -1)
