...
REGISTER_OP("QueryBallPoint")           // Register operation
    .Attr("radius: float")              // Attribute, radius of grouping
    .Attr("nsample: int")               // Attribute, = 16
    .Input("xyz1: float32")             // Input tensor of float32, point cloud 1
    .Input("xyz2: float32")             // Input tensor of float32, point cloud 2
    .Output("idx: int32")               // Output tensor of int32, indices of the points grouped
    .Output("pts_cnt: int32")           // Output tensor of int32, points count?
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {            // Output tensor is same shape as input tensor
        ::tensorflow::shape_inference::ShapeHandle dims2; // batch_size * npoint * 3
        c->WithRank(c->input(1), 3, &dims2);              // validates that the c->input(1) has a shape with exactly 3 dimensions
        int nsample;
        TF_RETURN_IF_ERROR(c->GetAttr("nsample", &nsample));       // Get the value of attribute nsample in variable nsample
        ::tensorflow::shape_inference::ShapeHandle output1 = c->MakeShape({c->Dim(dims2, 0), c->Dim(dims2, 1), nsample});  // Indices = batch size, n point, n sample, (16, 1024, 16)
        c->set_output(0, output1);
        ::tensorflow::shape_inference::ShapeHandle output2 = c->MakeShape({c->Dim(dims2, 0), c->Dim(dims2, 1)});          // Points count = batch size, npoint (16, 1024)
