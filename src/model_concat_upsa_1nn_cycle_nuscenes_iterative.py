...
def get_model(radius, layer, point_cloud, is_training, bn_decay=None, knn=False,
    ...
    # pc2_hat_coordinates
    pred_f_hat = pred_flow + point_cloud[:, :num_point, :3] # flow + p1
    # pc2_hat_coordinates_features
    pred_f_hat = tf.concat([pred_f_hat, point_cloud[:, :num_point, 3:]], axis = 2)
    # pc2_hat, pc2
    point_cloud_iter_2 = tf.concat([pred_f_hat, point_cloud[:, num_point:]], axis = 1)

    with tf.variable_scope(tf.get_variable_scope(), reuse=True):


        # pc2_double_hat
        pred_flow_iter_2, end_points = get_model_flow(radius, layer, point_cloud_iter_2, is_training,
                                                        bn_decay=None, knn=False,
                                                        flow_module='default')
        # pc2_double_hat_coordinates
        pred_f_hat_2 = pred_flow_iter_2 + pred_f_hat[:, :, :3]
        # pc2_double_hat_coordinates_features
        pred_f_hat_2 = tf.concat([pred_f_hat_2, pred_f_hat[:, :num_point, 3:]], axis = 2)
        # pc2_double_hat, pc2
        point_cloud_iter_3 = tf.concat([pred_f_hat_2, point_cloud[:, num_point:]], axis = 1)

        # pc2_triple_hat
        pred_flow_iter_3, end_points = get_model_flow(radius, layer, point_cloud_iter_3, is_training,
                                                        bn_decay=None, knn=False,
                                                        flow_module='default')
        # pc2_triple_hat_coordinates
        pred_f_hat_3 = pred_flow_iter_3 + pred_f_hat_2[:, :, :3]
        ...
        grouped_xyz = tf.squeeze(grouped_xyz, axis=2)
        end_points_f = {
            'idx': idx,
            'pred_flow': pred_flow_iter_3,
        ...
        if stop_gradient:
            ...
            tf.stop_gradient(pred_f_hat_3)


        pred_fb = tf.concat([pred_f_hat_3, point_cloud[:, :num_point, 3:]], axis = 2)
        ...
        pred_b = pred_flow_back + pred_f_hat_3
    ...
    return pred_f_hat_3, pred_b, grouped_xyz, end_points_f, end_points_b
