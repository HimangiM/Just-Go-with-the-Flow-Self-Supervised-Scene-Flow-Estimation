...
class SceneflowDataset():
    def __init__(self, root = './data_preprocessing/nuscenes_trainval_rgb_pkl',
        ...
        self.root = root
        if self.train:    # 20007
            self.datapath = glob.glob(os.path.join(self.root, 'train/*.pkl'))
        else:
            # print ('root here:', self.root)
            self.datapath = glob.glob(os.path.join(self.root, 'test/*.pkl'))
            # print('path here:', sorted(self.datapath))
        self.cache = {}
    ...
    def __getitem__(self, index):
        if index in self.cache:
            ...
            pc_list, rgb_list = pickle.load(open(fn, 'rb')) # list of point clouds
            ...
            for frame_idx in range(start_idx, start_idx + self.num_frames):
                ...
                rgb_data = rgb_list[frame_idx]/255.
